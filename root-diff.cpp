...
std::string createEventHeader(Long64_t eventIdx) {
    ...
    try {
        ...
        header = Color::BOLD_MAGENTA + "╔═══ Event " +
                 std::to_string(eventIdx) + " ═══╗" + Color::RESET;
    }
...
std::string buildInfoBox(const std::vector<std::string> &lines) {
    ...
    for (const auto &line : lines) {
        ...
        out << Color::CYAN << "│ " << Color::RESET << line
            << std::string(pad, ' ') << Color::CYAN << " │" << Color::RESET
            << "\n";
    }
    out << Color::CYAN << "╰" << horizontalLine(boxWidth - 2) << "╯"
        << Color::RESET;
...
void printCollectionBoxes(
    ...
    if (grouped.empty()) {
        std::vector<std::string> lines{Color::BOLD_YELLOW +
                                       std::string("(none)") + Color::RESET};
        printInfoBox(lines);
        return;
    }
...
bool compareFormattedValues(const std::string &val1, const std::string &val2,
                            bool isFloat1, bool isFloat2) {
    if (val1 == "*" || val2 == "*")
        return false;
    if (val1 == val2)
        return true;
    if (isFloat1 || isFloat2) {
        try {
            long double d1 = std::stold(val1);
            long double d2 = std::stold(val2);
            if (std::isnan(d1) && std::isnan(d2))
                return true;
            if (std::isinf(d1) && std::isinf(d2))
                return std::signbit(d1) == std::signbit(d2);
...
alignNumericStrings(const std::vector<std::string> &strings,
    ...
    for (const auto &s : strings) {
        if (s == "*") {
            ...
            result.push_back(
                std::string(std::max(0, width - (int)s.length()), ' ') + s);
        } else {
            size_t dotPos = s.find('.');
            if (dotPos != std::string::npos) {
                std::string intPart = s.substr(0, dotPos);
                std::string decPart = s.substr(dotPos + 1);
                result.push_back(std::string(maxInt - intPart.length(), ' ') +
                                 intPart + "." + decPart +
                                 std::string(maxDec - decPart.length(), ' '));
            } else {
                result.push_back(
                    std::string(maxInt - s.length(), ' ') + s +
...
void printPanelsInColumns(const std::vector<std::string> &panels) {
    ...
    for (size_t i = 0; i < panelLines.size(); i++) {
        ...
        if (panelWidth <= 0 || panelWidth > 1000) {
            debugPrint("Warning: Invalid panel width " +
        ...
        if (currentRow.empty() || currentWidth + panelWidth + 2 <= termWidth) {
            ...
            for (size_t ln = 0; ln < maxLines; ln++) {
                for (size_t j = 0; j < currentRow.size(); j++) {
                    ...
                    if (idx < panelLines.size() &&
                        ln < panelLines[idx].size()) {
                        std::cout << panelLines[idx][ln];
                    } else if (idx < panelLines.size() &&
                               !panelLines[idx].empty()) {
...
createReaderHandler(const std::string &branchName,
    ...
    for (size_t i = 1; i < trees.size(); i++) {
        ...
        if (typeId != typeId0 || isColl != isColl0) {
            debugPrint("Warning: Type or collection mismatch for " +
                       branchName);
            return nullptr;
        }
    }
...
void printUsage(const char *progName) {
    ...
    std::cout << "  -n, --num-events N         Number of events to compare "
                 "(default: -1, all)\n";
    std::cout
        << "  -s, --start N              Start from event N (default: 0)\n";
    std::cout << "  --abs-tolerance TOL        Absolute tolerance for floating "
                 "point\n";
    std::cout << "  --rel-tolerance TOL        Relative tolerance for floating "
                 "point\n";
...
std::vector<std::string> createAlignedPanels(
    ...
    for (const auto &[branchName, entries] : branchData) {
        ...
        for (const auto &e : filtered) {
            ...
            if (compact) {
                maxIdxWidth = std::max(
                    maxIdxWidth, (size_t)std::to_string(e.index).length());
            }
        }
    }
    ...
    for (const auto &[branchName, entries] : branchData) {
        ...
        std::string coloredTitle;
        {
            ...
            if (pos == std::string::npos) {
                ...
                coloredTitle = Color::CYAN + prefix + Color::RESET +
                               Color::BOLD_BLUE + suffix + Color::RESET;
            }
        }
        ...
        for (const auto &e : filtered) {
            ...
            for (size_t i = 0; i < fileCount; i++) {
                ...
                if (i + 1 < fileCount) {
                    ...
                    if (e.isDifferent) {
                        showArrow = !compareFormattedValues(
                            e.vals[i], e.vals[i + 1], e.isFloats[i],
                            e.isFloats[i + 1]);
                    }
                    if (showArrow) {
                        ls << "  " << Color::DIM << Color::CYAN << "->"
                           << Color::RESET << "  ";
                    } else {
...
int main(int argc, char **argv) {
    ...
    for (const auto &path : filePaths) {
        ...
        if (!file || file->IsZombie()) {
            std::cerr << Color::BOLD_RED
                      << "Error opening file: " << Color::RESET << path << "\n";
            if (file)
                file->Close();
            delete file;
            for (auto *f : files) {
    ...
    for (const auto &brs : branchesPerFile) {
        ...
        std::set_intersection(common.begin(), common.end(), brs.begin(),
                              brs.end(),
    ...
    if (REL_TOLERANCE >= 0) {
        infoLines.push_back("  Relative tolerance: " +
                            formatValue(REL_TOLERANCE));
    } else if (ABS_TOLERANCE >= 0) {
        infoLines.push_back("  Absolute tolerance: " +
                            formatValue(ABS_TOLERANCE));
    }
    ...
    if (common.empty()) {
        ...
        printCollectionBoxes(groupByCollection(common), Color::BOLD_GREEN +
                                                            std::string("✓") +
                                                            Color::RESET);
    }
    std::cout << "\n";
    ...
    if (evDiffs.empty()) {
        ...
        for (const auto &[ei, bds] : evDiffs) {
            ...
            for (const auto &[cn, brs] : colls) {
                ...
                std::vector<std::pair<std::string, std::vector<DiffEntry>>>
                    branchData;
